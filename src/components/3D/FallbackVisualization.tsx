...
const FallbackVisualization: React.FC<FallbackVisualizationProps> = ({ 
  ...
  return (
    <div className="w-full h-full relative bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 overflow-hidden rounded-lg">
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none">
        <defs>
          <linearGradient id="lineGradient" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <stop offset="100%" stopColor="#007fff" stopOpacity="0.2" />
          </linearGradient>
        </defs>
      ...
      {blocks.map((block, index) => (
        ...
        >
          ...
          <motion.div
            ...
            transition={{
              duration: 2,
              repeat: activeBlock === block.category ? Infinity : 0,
            }}
          />
      ...
      >
        <p className="text-slate-300 text-xs">
          <span className="text-cyan-400 font-medium">Interactive:</span> Click blocks to explore
        </p>
      </motion.div>
    </div>
  );
